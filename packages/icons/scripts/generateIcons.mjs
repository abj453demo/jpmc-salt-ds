...
const generateCssAsBg = ({ basePath, cssOutputPath, fileArg }) => {
  ...
  const fileNames = globSync(globPath, options);
...
const generateIconAll = async ({ icons, allPath }) => {
  ...
  const result = biomeFormat(joinedText, allPath);

  console.log(`Writing ${allPath}`);

  await fs.promises.writeFile(allPath, result, {
    encoding: "utf8",
  });
};
...
const generateIconAllSite = async ({ icons, siteAllPath }) => {
  ...
  const result = biomeFormat(joinedText, siteAllPath);

  console.log(`Writing ${siteAllPath}`);

  await fs.promises.writeFile(siteAllPath, result, {
    encoding: "utf8",
  });
};
